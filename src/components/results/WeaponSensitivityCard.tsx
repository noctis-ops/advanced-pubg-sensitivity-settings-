...
export function WeaponSensitivityCard({
  ...
  return (
    <div className="px-4 pb-4 space-y-3">
      ...
      {getWeaponProfile(result.weapon.id) && (
        <p className="text-[10px] text-amber-300/70">
          Domain profile: {getWeaponProfile(result.weapon.id)?.source} · not direct PUBG telemetry · vertical {Math.round((getWeaponProfile(result.weapon.id)?.verticalRecoilTendency ?? 0) * 100)}% · horizontal {Math.round((getWeaponProfile(result.weapon.id)?.horizontalRecoilTendency ?? 0) * 100)}%
        </p>
      )}
            ...
            return (
              <div key={value.scope} className="grid grid-cols-[145px_70px_70px_70px_78px_1fr] gap-2 px-3 py-3 border-t border-white/5 items-start">
                ...
                <div className="text-[10px] leading-relaxed text-gray-400">
                  ...
                  {value.optimization && <p className="mt-1 text-[9px] text-cyan-300/70">{isRTL ? 'Score' : 'Score'} {value.optimization.score.toFixed(1)} · {value.optimization.evidence} · {value.optimization.candidatesEvaluated} candidates</p>}
